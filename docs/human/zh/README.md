...
用你自己的 LLM 端点 + API key 登录。指向一个 git repo。Studio 编排 AI agent，把每个决策捕获为 ADR、每个失败捕获为 finding、每次 dispatch 记入 token ledger。M9 增加可选 `task_tag` dispatch 元数据，用来按任务类型分析 ledger 成本，同时不污染 provider 请求体。M10 增加可见的 `[ EN | 中 ]` UI 切换，login 与五个核心页面都可在英文/中文之间切换。
...
- **M9T/M9/M10 — v0.4.x 桌面 + ledger 元数据 + i18n**：Tauri shell、持久 session、`task_tag` ledger plumbing，以及 zh/en UI 切换。
